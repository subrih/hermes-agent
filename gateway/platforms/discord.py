    ...
    async def _process_voice_input(self, guild_id: int, user_id: int, pcm_data: bytes):
        ...
        try:
            ...
            from tools.transcription_tools import (
                transcribe_audio, transcribe_audio_live,
                get_stt_model_from_config, get_stt_provider_from_config,
            )
            stt_model = get_stt_model_from_config()
            stt_provider = get_stt_provider_from_config()
            if stt_provider == "gemini_live":
                result = await transcribe_audio_live(wav_path, model=stt_model)
            else:
